...
Pool support needs github-sts 0.1.1 or later, which is what this chart's `appVersion` pins. An older image rejects `instances:` outright instead of falling back to the flat form. The config decoder runs with known-fields checking, so the pod CrashLoopBackOffs with `field instances not found in type config.AppConfig`. Only an `image.tag` or `image.digest` override below the `appVersion` reaches that state.
...
| oidc.requireImmutableSubjectClaims | bool | `true` | Require GitHub.com Actions tokens to carry immutable owner and repository IDs inside the `sub` claim, in the form `repo:OWNER@OWNER-ID/REPO@REPO-ID:ref:refs/heads/BRANCH`. This is the broker's own default, so rendering it changes nothing until you set it to false.  It gates only what `sub` must contain. The separate signed `repository_id` and `repository_owner_id` claims are mandatory for every GitHub.com token either way, and the broker cross-checks them against the subject before audience, replay or policy work. Tokens from any other issuer are unaffected, and the immutable format does not exist on GitHub Enterprise Server.  Repositories created before 2026-07-15 keep the name-only subject format until they opt in through the organization or repository Actions OIDC setting, or the REST API. Until a repository has opted in, its exchanges are rejected with `githubsts_oidc_validation_errors_total{reason="github_identity_immutable_subject_required"}`.  Set false only as an explicit, temporary migration posture: the legacy name-only subject is vulnerable to repository namespace reuse, so an attacker who acquires an abandoned owner/repo name can present a subject that matches a trust policy written for the original. The broker logs a startup warning, reports `security.require_immutable_subject_claims: false` on `/health`, and drops the `githubsts_oidc_immutable_subject_claims_required` gauge to 0 so the posture is alertable. |
